...
PATHOLOGIE_MAPPING = {
    "Affections de longue durée (dont 31 et 32) pour d'autres causes": (
        "Affections longue durée (autres)"
    ),
    "Cancers": "Cancers",
    "Diabète": "Diabète",
    ...
    (
        ...
        "(avec ou sans pathologies, traitements ou maternité)"
    ): "Hospitalisations diverses",
    ...
    "Maladies du foie ou du pancréas (hors mucoviscidose)": (
        "Maladies hépatiques/pancréatiques"
    ),
    ...
    (
        "Pas de pathologie repérée, traitement, maternité, hospitalisation "
        "ou traitement antalgique ou anti-inflammatoire"
    ): "Aucune pathologie repérée",
    (
...
def clean_csv_data(
    ...
    try:
        ...
        count_kept = len(df)
        reporter(
            f"[INFO] Nettoyage CSV terminé : {count_kept} lignes conservées sur {initial_rows}."
        )
